...
(function () {
  ...
  if (!isBot && store('l', 'fz-own') !== '1' && !store('s', 'fz-v')) {
    store('s', 'fz-v', '1');
  ...
  function open() {
    ...
    if (store('l', 'fz-own') === null) store('l', 'fz-own', '1'); // first open: stop counting this device
    panel = document.createElement('div');
    panel.setAttribute('role', 'dialog');
    panel.style.cssText = 'position:fixed;right:16px;bottom:16px;z-index:9999;max-width:320px;width:calc(100% - 32px);' +
      'background:#132A3D;color:#eaf1f8;border-radius:14px;padding:16px 18px;font:14px/1.5 -apple-system,Helvetica,Arial,sans-serif;' +
      'box-shadow:0 18px 44px rgba(0,0,0,.35)';
    panel.innerHTML =
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">' +
        '<b style="font-size:15px">Návštevnosť forensika.eu</b>' +
    ...
    cb.checked = store('l', 'fz-own') === '1';
    cb.onchange = function () { store('l', 'fz-own', cb.checked ? '1' : '0'); };
